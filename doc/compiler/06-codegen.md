...
| `sums.rs` | Sum-type storage layout: `register_sums_opaque` + `register_sum_bodies` fill the `sum_types` cache with `{ i32 tag, [k x iN] }` sized/aligned to the largest variant *payload* struct — the payload sits at one uniform offset for every variant (never in the tag's padding), because first-class aggregate copies preserve fields, not padding. Opaques are created *before* struct bodies (a struct field may hold a sum by value); bodies are computed *after* them, iterating to fixpoint for sum-in-sum payloads — by-value cycles were already rejected at parse time, so each round makes progress. `sizeof` and value lowering resolve sums through this cache exactly as structs use `struct_types`; `SumConstruct` lowers via a temp alloca (tag store through the storage type, payload stores through the variant's bare payload struct at field 1, array payloads memcpy'd) and loads the whole storage value, which then flows like any struct value (`is_struct_value` covers sums for `sret`/`byval`) |
