...
| `switch.rs` | `generate_switch`: scrutinee evaluated once (sums into an entry-block slot, tag loaded from field 0; bool normalized to `i1`), one LLVM `switch` over the discriminant, per-arm blocks copy bound payload fields out of the slot through the variant's payload struct (arrays memcpy'd) into fresh locals; the else edge is the `default` block, or a `llvm.trap` + `unreachable` block on a fully-listed enum/sum switch (forged tags halt — and `llvm.trap`, never libc `abort`, keeps freestanding targets libc-free) |
...
| `sums.rs` | Sum-type storage layout: `register_sums_opaque` + `register_sum_bodies` fill the `sum_types` cache with `{ tag, [k x iN] }` (tag = smallest int fitting the variant count, via `sum_tag_type` — the single authority every tag load/store/constant must go through) sized/aligned to the largest variant *payload* struct — the payload sits at one uniform offset for every variant (never in the tag's padding), because first-class aggregate copies preserve fields, not padding. Opaques are created *before* struct bodies (a struct field may hold a sum by value); bodies are computed *after* them, iterating to fixpoint for sum-in-sum payloads — by-value cycles were already rejected at parse time, so each round makes progress. `sizeof` and value lowering resolve sums through this cache exactly as structs use `struct_types`; `SumConstruct` lowers via a temp alloca (tag store through the storage type, payload stores through the variant's bare payload struct at field 1, array payloads memcpy'd) and loads the whole storage value, which then flows like any struct value (`is_struct_value` covers sums for `sret`/`byval`) |
