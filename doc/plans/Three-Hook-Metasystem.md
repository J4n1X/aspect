...
- **Idempotence of re-checking is the load-bearing assumption** — verified today (the checker only stamps `expr_type` and narrows literals; it never restructures the AST) and guarded permanently by a regression test: check, clone the `Program`, re-check the clone with a fresh checker, `assert_eq!`.
...
- [ ] **Attributes** (M) — inert cargo; `@` is a hard lexer error today, so nothing regresses. No warning for unclaimed attrs (no warning channel exists; the Phase 2b hygiene rule makes them hard errors later). Only syntactic validation: `@` must be followed by an identifier.
...
- [ ] **Checker-resolved `MethodCall`** (M) — §14.2. `ExprKind::MethodCall` resolved and mangled by the typechecker, not the parser. Required before Phases 3/4, not before Phase 2.
