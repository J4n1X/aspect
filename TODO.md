...
|-------------------------------|---------------------------------------------------------------------------------------------|----------|
| Two-pass prototype registration | Kill the no-forward-references limitation: pass 1 scans the token stream and registers every function prototype, type-struct, and alias; pass 2 parses bodies against the complete symbol table. More elegant than C-style forward declarations, and a **prerequisite for `$import`** (makes module/import order non-semantic; enables mutual recursion). Precedent: type-struct names already get a name-collection prescan. See doc/plans/Preprocessor-Infrastructure.md § 2. | HIGH |
| Preprocessor directives + modules | `$define`/`$undefine`, `$ifdef`/`$if`/`$elseifdef`/`$elseif`/`$else`/`$endif`, `$module`/`$import` with `-I` search roots and `-D` CLI defines; `$include` removed. `@` stays reserved for the metasystem (attributes/transforms). Full plan: doc/plans/Preprocessor-Infrastructure.md. | HIGH |
